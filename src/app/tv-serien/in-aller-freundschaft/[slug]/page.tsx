...
const SERIES_ID = 'in-aller-freundschaft';
const SERIES_LABEL = 'In aller Freundschaft';
...
export async function generateStaticParams() {
  ...
  return series
    .filter((s) => s.entry.seriesId === SERIES_ID && s.entry.status !== 'draft')
    .map((s) => ({ slug: s.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    alternates: { canonical: `${SERIES_PATH}/${slug}` },
    openGraph: {
      title: seoTitle || article.title,
      description: seoDescription || article.excerpt,
...
export default async function SeriesArticle({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;

  const article = await reader.collections.series.read(slug, { resolveLinkedFiles: true });
  if (!article) notFound();
  if (article.seriesId !== SERIES_ID) notFound();
